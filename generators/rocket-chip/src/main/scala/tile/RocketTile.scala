...
/**
 * Notes(DD): tile should be the top module I shall investigate
 * Some quick notes：
 * 1. The tile will include a ptw instance because of HasLazyRoCCModule
 * 2. The tile will also include our pmptw instance for the same reason
 * 3. In this module, ptw.io.dpath is connected to core.io.ptw (RocketCore): DataPathIO
 * 4. In this module, ptw.io.requestor is connected to ptwPorts: TLBIO
 * */
class RocketTileModuleImp(outer: RocketTile) extends BaseTileModuleImp(outer)
  ...
  //TODO(DD): comment the below checking as we introduce pmptw now
  ...
  // DD: connects the requests between pmptw and dcache interface
  pmptw.io.requestor <> pmptwPorts
}
