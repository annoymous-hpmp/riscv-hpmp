...
class DCacheModule(outer: DCache) extends HellaCacheModule(outer) {
  ...
  io.ptw <> tlb.io.ptw
  io.pmptw <> tlb.io.pmptw // matching pmptw between TLB and DCache
  ...
  when (!tlb.io.req.ready && !tlb.io.ptw.resp.valid && !tlb.io.pmptw.resp.valid && !io.cpu.req.bits.phys) { io.cpu.req.ready := false }
